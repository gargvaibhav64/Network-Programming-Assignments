...
void create_pipeline3(char *input, pipeline *pipeline, int flag)
{
    ...
    len = strlen(token);
    str2 = malloc(len + 1 + 1 ); /* one for extra char, one for trailing zero */
    strcpy(str2, token);
    str2[len] = c;
    str2[len + 1] = '\0';
    ...
    parse_cmd(&cmd4, token);

    if (cmd4->ip_redirect && is_fd(cmd4->ip_file))
    {
        int ip_fd = atoi(cmd4->ip_file);
        // Work reqd
    }

    insert_cmd(pipeline, cmd4);

    if (cmd4->op_redirect && is_fd(cmd4->op_file))
    {
        int op_fd = atoi(cmd4->op_file);
        // Work reqd
    }
